/*
Target: WheelChair with Kinect, Leap Motion,Atmega2560
Author: Tuo Yao
*/ 
/*---------------------------------------------------------------------------------------------------------*/
//Load Library//
...
void draw()
{
 ...
 /*-----------------------------------------------------------------------------*/
  ...
  if(Mode_Flag == 1)
  {
    ...
    if (userList.size()>0)   //如果检测到用户
    {
      userID = userList.get(0);  // 检测到的第一个用户
      if (kinect.isTrackingSkeleton(userID))   //kinect 开始追踪第一个用户
      {
      	stroke(255, 0, 0);
        fill(255, 0, 0);
        ...
        PVector rightHand2D_XY     = new PVector(rightHand.x, rightHand.y);       //创建各个关节2维坐标
        ...
        PVector differenceVector  = PVector.sub(leftHand,rightHand);        //向量减法  sub函数
        PVector differenceVector1 = PVector.sub(rightElbow, rightShoulder);    //Right Large Arm
        
        float magnitude = differenceVector.mag();    //计算向量长度  mag函数
        float magnitude1 = differenceVector1.mag();  
        
        differenceVector.normalize();    //使其成为单位向量
        ...
        offset = int(map(torsoPosition.x, -700, 700, -15,15));    //Offset 
        SpeedFollowing = int(map(torsoPosition.z, 1800, 3000, 10, 70));   //Speed with Distance
        ...
        /*----------------------------*/
        if(rightHand.y > rightShoulder.y)   // 右手
        {
        	text("START",15,570);
...
/*---------------------------------------------------------------------------------------------------------*/
void drawJoint(int userID, int jointID)             // 画  关节
{
...
float angleOf(PVector one, PVector two, PVector axis)
{
  ...
  return degrees(PVector.angleBetween(limb, axis));  // angleBetween 计算两个向量之间的弧度  degrees 将弧度转化为角度
}
/*---------------------------------------------------------------------------------------------------------*/
...
void Send_Data()
{
  out[0] = byte(int(SpeedL * 0.85));
  out[1] = byte(int(SpeedR * 0.85));
